...
TEST_CASE("tds::protocol::Sender", "[protocol]") {
    const ip::Port server_port{46798};
    ...
    std::array<char, message.size()> client_buffer;
    const ssize_t count = read(client_fd, client_buffer.data(), client_buffer.size());
    REQUIRE(count == message.size());
    REQUIRE(std::ranges::equal(message, client_buffer));
    close(client_fd);
}
